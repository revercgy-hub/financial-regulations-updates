...
原生 Android 查询 APP。联网版首次启动从 GitHub Release 下载完整知识库；后续版本优先只下载新增、修改和删除清单，在本机安全合并。只有本地版本跨过了增量起点或增量校验失败时，才自动回退下载完整包。Android 在联网时每天执行一次后台检查。
...
- 联网同步案例，来源包括财政部、证监会、审计署、中央纪委国家监委，数量以服务器最新清单为准。
- APP 内切换金融监管制度库、会计制度库和案例库；案例支持机构、主体、处理类型、案由和通告阶段筛选。
- 下载文件大小和 SHA-256 双重校验。
- 增量包逐文件 SHA-256 校验；本地合并使用硬链接快照，设备不支持时自动改用文件复制。
- 临时目录解压、原子替换和上一版本回滚。
...
输出：`FinReg-KnowledgeBase-Online-v1.8.0.apk`
...
输出示例：`FinReg-KnowledgeBase-Offline-KB20260807.1-v1.8.0.apk`
...
清单指定知识库版本、制度和案例数量、四类案例来源统计、完整包与增量包下载地址、文件大小、SHA-256 和最低 APP 版本。
